...
// These two are just helpers, they curate spring data, values that are later being interpolated into2 css
const to2 = i => ({ x: 0, y: i * -4, scale: 1, rot: -10 + Math.random() * 20, delay: i * 100 })
const from = i => ({ x: 0, rot: 0, scale: 1.5, y: -1000 })
// This is being used down there in the view, it interpolates rotation and scale into2 a css transform
...
function Deck() {
  ...
  const bind = useDrag(({ args: [index], down, delta: [xDelta], distance, direction: [xDir], velocity }) => {
    ...
    if (!down && gone.size === cards.length) setTimeout(() => gone.clear() || set(i => to2(i)), 600)
  })
  // Now we're just mapping the animated values to2 our view, that's it. Btw, this component only renders once. :-)
